...
class ServerStatus
{
    ...
    public static function query($host = '127.0.0.1', $port = 25565)
    {
        ...
        if (is_object($descriptionRaw) && isset($descriptionRaw->extra)) {
            $description = '';
            foreach ($descriptionRaw->extra as $item) {
                $description .= (substr($item->text, 0, 1) === "\n") ? "<br>" : "";
                $description .= isset($item->bold) && $item->bold ? '<b>' : '';
                $description .= '<span style="color:'. (isset($item->color) ? $item->color : "white").'">' . $item->text . '</span>';
                $description .= isset($item->bold) && $item->bold ? '</b>' : '';
            }
        }
        return array(
            ...
            'players' => isset($data->players) ? $data->players : false,
